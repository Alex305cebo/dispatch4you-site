# Обновление ссылок в файлах pages/

Write-Host "Обновление ссылок в папке pages/..." -ForegroundColor Green
...
# Замены для файлов в папке pages/
$replacements = @{
...
foreach ($file in $pagesFiles) {
    ...
    Write-Host "Обновляю $($file.Name)..." -ForegroundColor Yellow
